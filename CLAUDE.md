...
A ComfyUI custom node for organising, composing and reusing text prompts. ~5.5k
...
Dependencies point one way. Nothing below the line knows about aiohttp or the DOM.
...
web/preset_loader.js   the canvas node widget
web/browse.js          the standalone library page
web/preset_composer.js picker, parts editor, preset editor, part creator
web/preset_dialog.js   openDialog / openPopover — every overlay
web/preset_store.js    shared snapshot + SSE subscription
web/preset_api.js      fetch wrappers
web/preset_dnd.js      pointer drag + edge autoscroll
web/preset_model.js    mirror of preset_model.py
web/preset_icons.js    Lucide icons, pathTone, element() — imports nothing
web/preset_ui.css      all component styles + design tokens
web/browse.css         browse-page-only styles
```
...
**`os.fsync()` on Windows needs a write handle.** It maps to `_commit()` and
raises `EBADF` on a read-only one. Open backups `"r+b"`, not `"rb"`. This broke
every save on Windows once already.

**`ALLOWED_ASSETS` must list every module in browse.js's import graph.**
browse.js imports by absolute URL through `/preset_loader/assets/`, which is a
whitelist. A missing entry is a 404 at runtime, not an error at build time — and
the constant is read at import, so **restart the server after editing it**.
...
**`preset_model.py` and `preset_model.js` must stay in sync.** Same shape rules,
same name rules, same error strings — the client validates so the user gets an
inline message instead of a failed POST. There is a differential test approach
that feeds a shared key corpus through both and compares; use it after touching
either.
...
**No build step.** ComfyUI serves `web/` verbatim and only loads `.js`. TypeScript,
JSX or bundling would require committing build output or breaking git-clone
installs. If type safety is wanted, use `// @ts-check` + JSDoc.
...
`preset_icons.js` rather than fetched.

**`preset_icons.js` must not import anything.** `preset_dialog` needs `iconSvg`
and `preset_composer` needs `openDialog`; the leaf module is what stops that
becoming a load-order-dependent cycle.
...
no keyboard path. `preset_dnd.js` therefore matters: it autoscrolls on a
...
- No inline `style.cssText`. Styles belong in `preset_ui.css`. Inline styles are
